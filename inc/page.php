...
$can_view_page = true;
$can_edit_post = dcCore::app()->auth->check('contentadmin,pages', dcCore::app()->blog->id);
$can_publish = dcCore::app()->auth->check('contentadmin', dcCore::app()->blog->id);
$can_delete = false;
$preview = false;
    ...
    $sidebar_items = new ArrayObject(array(
        'status-box' => array(
            ...
            'items' => array(
                ...
                dcCore::app()->adminurl->get('admin.plugin.related', array('do' => 'edit', 'id' => $post_id,'xconv' => '1')).
                '">'.
