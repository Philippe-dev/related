...
dotclear.viewPostContent = (line, _action = 'toggle', e = null) => {
    ...
    const postId = $(line).attr('id').substring(1); const lineId = `pe${postId}`; let tr = document.getElementById(lineId);
    if (tr) {
        $(tr).toggle();
        ...
        dotclear.getEntryContent(postId, (content) => {
            if (content) {
                ...
                tr.id = lineId; const td = document.createElement('td');
...
dotclear.ready(() => {
    ...
    dotclear.expandContent({ line: document.querySelector('#form-entries tr:not(.line)'), lines: document.querySelectorAll('#form-entries tr.line'), callback: dotclear.viewPostContent, });
    ...
    $('#pageslist tr.line td:not(.expander)').on('mousedown', () => {
        $('#pageslist tr.line').each(
            function () {
                const td = this.firstChild; dotclear.viewPostContent(td.firstChild, td.firstChild.line, 'close');
            });
